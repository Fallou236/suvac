...
\fancyfoot[L]{\sffamily\scriptsize\color{neutral}Version 1.1 — Septembre 2026}
...
\midrule
\textbf{Agent de santé} & Infirmier, sage-femme ou matrone du poste de santé. Enregistre les bénéficiaires et administre les doses. Utilisateur principal, souvent en connectivité dégradée.  Ouvre et ferme l'accès des mères à leur espace personnel. & PWA, mode hors ligne \\
\rowcolor{softbg}
\textbf{Mère ou tuteur} & Bénéficiaire finale. Consulte le carnet de ses enfants et de ses grossesses, reçoit les rappels accompagnés de l'explication de chaque vaccin. Peut être non alphabétisée. N'enregistre aucun acte vaccinal. & Rappels vocaux par WhatsApp ; espace personnel en consultation, si l'agent lui ouvre un accès \\
...
\rid{EF-12} & L'agent enregistre un enfant rattaché à une mère : identité, date de naissance, sexe, prématurité éventuelle. Le prénom peut être absent — au Sénégal, il est souvent donné au baptême, après le BCG ; le rattachement à la mère suffit à identifier le bénéficiaire. & \pM \\
\rowcolor{softbg}
...
\rowhead \hd{Réf.} & \hd{Exigence} & \hd{Prio.} \\
\midrule \endhead
\rid{EF-70} & L'interface professionnelle est disponible en français ; les messages adressés aux bénéficiaires sont en wolof. Le mécanisme d'internationalisation permet d'étendre la traduction de l'interface si le besoin s'en confirme. & \pM \\
\rowcolor{softbg}
...
\subsection{Espace bénéficiaire}

\begin{xltabular}{\textwidth}{@{}C{1.5cm} L C{1.3cm}@{}}
\toprule
\rowhead \hd{Réf.} & \hd{Exigence} & \hd{Prio.} \\
\midrule \endhead
\rid{EF-80} & L'agent ouvre et ferme l'accès d'une mère à son espace personnel. Fermer l'accès désactive le compte sans supprimer l'historique. & \pM \\
\rowcolor{softbg}
\rid{EF-81} & La mère consulte le carnet de chacun de ses enfants et de chacune de ses grossesses, regroupé par étape d'âge. & \pM \\
\rid{EF-82} & La mère voit en un écran ce qui est à faire, ce qui est en retard et ses prochains rendez-vous, pour elle et ses enfants. & \pM \\
\rowcolor{softbg}
\rid{EF-83} & La mère reçoit ses rappels dans l'application ; un retard est signalé sans culpabiliser. & \pM \\
\rid{EF-84} & Chaque vaccin, dans le carnet comme dans les messages, est accompagné d'une explication destinée aux mères. & \pM \\
\rowcolor{softbg}
\rid{EF-85} & Les fiches des vaccins sont consultables par tous les utilisateurs connectés. & \pS \\
\rid{EF-86} & La mère peut corriger son téléphone et son village, jamais son identité. & \pS \\
\bottomrule
\end{xltabular}
...
\rowcolor{softbg}
\rid{RG-05bis} & Une échéance dont la fenêtre de rattrapage est close passe au statut « périmée » : la dose n'est plus administrable. Le seuil entre « due » et « en retard » suit la fenêtre propre à chaque vaccin, au tiers de celle-ci. \\
...
\begin{vigilance}[Vérification manuelle d'ENF-15]
La limitation de débit est neutralisée pendant les tests : les compteurs de
DRF résident dans un cache partagé entre les cas de test, alors que la base
est réinitialisée à chaque fois. Elle est donc vérifiée manuellement. Deux
tentatives d'automatisation ont été écartées, l'une butant sur ce cache,
l'autre sur la configuration mise en cache par DRF.
\end{vigilance}
...
\rid{ENF-25} & Une bénéficiaire n'accède qu'à son propre dossier. Un identifiant valide mais rattaché à une autre bénéficiaire reçoit la même réponse qu'un identifiant inconnu. \\
\rowcolor{softbg}
...
Unitaire frontend & Composants, hooks, logique de synchronisation. \texttt{Vitest}, RTL, \texttt{MSW} & $\geq$ 70 \% (82,7 \% atteints) \\
\rowcolor{softbg}
...
Typage Python & \texttt{mypy} & progressif, bloquant à partir du sprint 3 \\
\rowcolor{softbg}
...
\ganttbar{Sprint 1 — Backend et API}{3}{4} \\
\ganttbar{Sprint 2 — Frontend React}{5}{6} \\
...
\rowcolor{softbg}
\rid{S-1} & Backend métier et API & Modèle de données, moteur de calendrier et sa suite de tests, référentiel vaccinal, API REST complète \\
\rowcolor{softbg}
\rid{S-2} & Frontend React & Socle, authentification, écrans bénéficiaires, consultation des calendriers, saisie des doses \\
...
\rowcolor{softbg}
1.1 & Septembre 2026 & Fallou Diouck & Découpage des sprints révisé (13.3) ; \texttt{mypy} rendu progressif (11.4) ; vérification manuelle d'ENF-15 (7.2) ; prénom facultatif pour l'enfant (EF-12) \\
 &  &  &  \\
\rowcolor{softbg}
1.2 & Septembre 2026 & Fallou Diouck & Périmètre de l'internationalisation précisé (EF-70, ADR 0005) ; statut « périmée » ajouté au moteur (RG-05bis) ; couverture frontend atteinte \\
 &  &  &  \\
\rowcolor{softbg}
1.3 & Septembre 2026 & Fallou Diouck & Espace bénéficiaire (EF-80 à EF-86, ENF-25, ADR 0006) ; fiches vaccins ; tableau de bord superviseur livré ; écran de paramètres \\
\bottomrule
\end{tabularx}
...
{\sffamily\footnotesize\color{neutral}
Fin du document — \textbf{SUVAC}, cahier des charges, version 1.1}
\end{center}
