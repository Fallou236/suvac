...
def test_serie_recente_n_est_pas_abandonnee():
    ...
    assert not serie_abandonnee(
        code_vaccin="PENTA", calendrier=resultat, aujourdhui=date(2026, 5, 1)
    )
...
def test_serie_jamais_commencee_n_est_pas_abandonnee():
    ...
    assert not serie_abandonnee(
        code_vaccin="PENTA", calendrier=resultat, aujourdhui=date(2027, 6, 1)
    )
...
def test_schema_incoherent_est_rejete_a_la_construction():
    with pytest.raises(ValueError):
        RegleVaccinale(code_vaccin="X", libelle="X", rang=1, age_min_jours=60, age_cible_jours=30)

    with pytest.raises(ValueError):
        RegleVaccinale(code_vaccin="X", libelle="X", rang=0, age_min_jours=0, age_cible_jours=0)
